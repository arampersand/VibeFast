...
Los bloques azulados son <Term id="prompt">prompts</Term> listos para copiar y pegar en <Term id="cursor">Cursor</Term>:
...
Los bloques grises con fondo oscuro son <Term id="comando">comandos</Term> de <Term id="terminal">terminal</Term>. Tienen un botón de copiar en la esquina — úsalo en vez de seleccionar el texto a mano, así evitas copiar espacios de más.
...
Ejemplo clásico, al clonar tu <Term id="repo-repositorio">repo</Term>:

```bash
git clone https://github.com/TU-USUARIO/VibeFast.git
```

Aquí `TU-USUARIO` **no es el usuario real** — es un ejemplo. Tienes que cambiarlo por tu usuario de <Term id="github">GitHub</Term> antes de correr el comando. Si lo pegas tal cual, va a fallar porque esa carpeta no existe.
...
| Ejemplo | Para qué |
| --- | --- |
| Comando → qué hace | Cheat-sheet de terminal |
| Servicio → dónde sacar la key | Setup de cuentas |

...
- ✅ Waitlist funcionando en tu landing, guardando emails en tu <Term id="base-de-datos">base de datos</Term>.
- ✅ Todo <Term id="deploy">deployado</Term> en <Term id="vercel">Vercel</Term> con una **URL pública**.
...
2. Instalar dependencias y crear tu `web/.env.local` con las 3 keys de <Term id="supabase">Supabase</Term>.
3. Subir el <Term id="schema">schema</Term> a Supabase (Paso 6 — con esto tu waitlist ya guarda de verdad).
4. Correr el proyecto con `yarn dev`.

**→ Sigue [Empieza aquí → Quick start](/docs/setup/quick-start) hasta el Paso 8** (30–60 min). Si no adelantaste tu proyecto de Supabase en Semana 0, el propio Quick start te manda a crearlo antes del Paso 1.
...
`web/config.js` es **el único archivo** que hay que cambiar para que la landing sea tuya — y lo cambia <Term id="cursor">Cursor</Term> por ti. Copia este prompt y rellena las llaves con los datos de tu ficha de Semana 0:
...
¿El formulario da error o la fila no aparece? Casi siempre es que faltó el **Paso 6 del Quick start** (subir el schema) o una key mal pegada en `web/.env.local` — revisa [Errores comunes](/docs/troubleshooting/errores-comunes).
</Checkpoint>
...
1. Importar tu <Term id="fork">fork</Term> en Vercel (login con GitHub).
2. Poner **Root Directory = `web`** (crítico — es monorepo).
3. Pegar tus <Term id="variable-de-entorno">variables de entorno</Term> (las 4 mínimas).
4. Deployar, y luego actualizar `NEXT_PUBLIC_APP_URL` con la URL real + redeploy.

**→ Sigue el [Paso 9 del Quick start](/docs/setup/quick-start#paso-9--deploy-a-vercel)** (~10 min).
